...
This guide walks you through the steps to self-host datagusto platform using Docker Compose.

<b>Requirements</b>:
- git
- docker & docker compose (use Docker Desktop on Windows or MacOS)

<b>Self-Hosting Deployment</b>:
...
2. Configure environment variables:
    ...
    - `JWT_REFRESH_SECRET_KEY`: Secret key for JWT refresh tokens (generate with `openssl rand -hex 32`)

    Optional configurations:
    - `POSTGRES_PORT`: PostgreSQL port (default: 5432)
    - `BACKEND_PORT`: Backend API port (default: 8000)
    - `FRONTEND_PORT`: Frontend web UI port (default: 3000)
    - `ENABLE_REGISTRATION`: Allow user registration (default: false)
...
All database data is stored in a Docker volume named `postgres_data`. To completely reset the application:
```bash
docker compose down -v
```

<b>Security Notes</b>:
- Change all default passwords and secret keys in `.env` before deploying to production
- Consider using a reverse proxy (nginx, Traefik) with SSL/TLS certificates for public deployments
- Set `ENABLE_REGISTRATION=false` in production and manually create user accounts
