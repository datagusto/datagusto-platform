...
async def reanalyze_trace_quality(
    ...
    try:
        ...
        (
            quality_score,
            quality_issues,
        ) = await DataQualityService.analyze_trace_data_quality(
...
async def get_project_quality_summary(
    ...
    try:
        ...
        avg_quality = sum(t.quality_score for t in traces_with_quality) / len(
            traces_with_quality
        )
        ...
        quality_distribution = {
            "excellent": len(
                [t for t in traces_with_quality if t.quality_score >= 0.9]
            ),
            "good": len(
                [t for t in traces_with_quality if 0.7 <= t.quality_score < 0.9]
            ),
            "fair": len(
                [t for t in traces_with_quality if 0.5 <= t.quality_score < 0.7]
            ),
