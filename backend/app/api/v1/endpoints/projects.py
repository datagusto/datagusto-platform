...
async def create_project(
    ...
    if not await org_member_repo.is_admin_or_owner(
        current_user.id, project_data.organization_id
    ):
        raise HTTPException(
            status_code=status.HTTP_403_FORBIDDEN,
...
async def get_project_api_key(
    ...
    return ApiKeyResponse(
        api_key=project.api_key, message="API key retrieved successfully"
    )
