...
class TraceRepository:
    ...
    async def get_traces_by_agent_id(self, agent_id: str) -> List[TraceModel]:
        ...
        Returns:
            List of traces
        """
        return self.db.query(TraceModel).filter(
            TraceModel.agent_id == agent_id
        ).order_by(TraceModel.started_at.desc()).all()
    
    async def update_trace(self, trace_id: str, trace_data: Dict[str, Any]) -> Optional[TraceModel]:
        """
        Update a trace.
    ...
    async def add_observation(self, observation_data: Dict[str, Any]) -> ObservationModel:
        """
    ...
    async def get_observations_by_trace_id(self, trace_id: str) -> List[ObservationModel]:
        """
        ...
        """
        return self.db.query(ObservationModel).filter(
            ObservationModel.trace_id == trace_id
        ).order_by(ObservationModel.started_at.asc()).all() 