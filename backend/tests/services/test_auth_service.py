...
async def test_register_user_success(
    ...
    org_data = OrganizationDataFactory.build(org_id=org_id, name="Test Organization")
    ...
    mock_organization_repository.create.return_value = build_mock_organization_model(
        org_data
    )
...
async def test_login_user_success(
    ...
    - User status checks passed (active, not archived)
    ...
    with (
        ...
        result = await auth_service.login_user(
            email=user_data["email"], password="Test123!@#"
        )
    ...
    # Verify repository calls
    mock_user_repository.get_by_email_with_relations.assert_called_once_with(
        user_data["email"]
    )
...
async def test_login_user_invalid_password(
    auth_service, mock_user_repository, mock_user_status_repository
):
    """
